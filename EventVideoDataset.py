...
class EventVideoDetectionDataset(Dataset):
    ...
    def _getClips(self):
        ...
        if not self.video_path_h5:
            raise FileNotFoundError(f"No .h5 files found in {self.video_path}")
        self.label_files = sorted(
            glob.glob(
                os.path.join(self.video_path.replace("images", "labels"), "*.npy")
            )
        )
        ...
        for i, f in enumerate(self.label_files):
            ...
            video_info = [
                {"frame": begin + indexes[idx], "labels": labels[idx], "label_file": f}
    ...
    def _extractClips(self, video, length):
        ...
        if self.load_type == "train" or self.mode == "batched":
            ...
            required_length = (self.num_clips - 1) * (
                self.clip_stride
            ) + self.clip_length
            indices = np.tile(np.arange(0, required_length, 1, dtype="int32"), 1)
            # Starting index of each clip
            clip_starts = np.arange(0, len(indices), self.clip_stride).astype("int32")[
                : self.num_clips
            ]
            ...
            if self.sequence_last_clip == []:
                ...
                self.sequence_last_clip.append(
                    self.sequence_last_clip[len(self.sequence_last_clip) - 1]
                    + self.num_clips
                )

            # print(clip_starts)
            final_video = [
                video[_idx : _idx + self.clip_length] for _idx in clip_starts
            ]
        else:
            self.num_clips = 1
            self.clip_stride = 1
    ...
    def __getitem__(self, idx):
        ...
        frame_ind = [
            [idx5] * len(vid_info[idx5]["labels"]) for idx5 in range(len(vid_info))
        ]
        flat_frame_ind = [item for sublist in frame_ind for item in sublist]
        cur_label = np.array(
            [self.data_file["1mp"][s, :, :, :] for s in frame]
        )  # for idx6 in range(len(vid_info))]
        ...
        ret_dict["cls"] = torch.from_numpy(
            np.concatenate(classes, axis=0, dtype=np.float64, casting="unsafe")
        )
    ...
    def collate_fn_val(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k == "img":
                ...
                fill = torch.Tensor(
                    [
                        value[0].shape[0] / value[i].shape[0]
                ...
                if fill_mask.any():
                    ...
                    value = list(value)
                    value[idx:] = [
                        torch.cat([value[idx + k1]] * int(fill[idx + k1]), 0)
                        for k1 in range(0, fill_mask.sum())
                    ]
                    value = tuple(value)

                    values[5] = list(values[5])
                    values[5][idx:] = [
                        torch.cat([values[5][idx + k1]] * int(fill[idx + k1]), 0)
                        for k1 in range(0, fill_mask.sum())
                    ]
                    ...
                    values[1][idx:] = [
                        torch.cat([values[1][idx + k1]] * int(fill[idx + k1]), 0)
                        for k1 in range(0, fill_mask.sum())
                    ]
                    ...
                    values[2][idx:] = [
                        torch.cat([values[2][idx + k1]] * int(fill[idx + k1]), 0)
                        for k1 in range(0, fill_mask.sum())
                    ]
                    ...
                    values[7][idx:] = [
                        torch.zeros(values[2][idx + k1].shape)
                        for k1 in range(0, fill_mask.sum())
                    ]
                    ...
                    sequence_mask = [
                        values[6][idx + k1]
                        < (value[idx + k1].shape[0])
                        - (value[idx + k1].shape[0]) / int(fill[idx + k1])
                        for k1 in range(0, fill_mask.sum())
                    ]
                ...
                if dif_mask.any():
                    ...
                    value[idx:] = [
                        torch.cat(
                            [
                                value[idx + k1],
                                value[idx + k1][: dif[idx + k1].type(torch.int32)],
                            ],
                            0,
                        )
                        for k1 in range(0, dif_mask.sum())
                    ]
                    value = tuple(value)
                    ...
                    values[5][idx:] = [
                        torch.cat(
                            [
                                values[5][idx + k1],
                                values[5][idx + k1][sequence_mask[k1]]
                                + value[idx + k1].shape[0],
                            ],
                            0,
                        )
                    ...
                    values[1][idx:] = [
                        torch.cat(
                            [
                                values[1][idx + k1],
                                values[1][idx + k1][sequence_mask[k1]],
                            ],
                            0,
                        )
                        for k1 in range(0, dif_mask.sum())
                    ...
                    values[2][idx:] = [
                        torch.cat(
                            [
                                values[2][idx + k1],
                                values[2][idx + k1][sequence_mask[k1]],
                            ],
                            0,
                        )
                        for k1 in range(0, dif_mask.sum())
                        ...
                        torch.cat(
                            [
                                ...
                                torch.Tensor(
                                    np.arange(
                                        values[6][idx + k1][-1] + 1,
                                        values[6][idx + k1][-1] + 1 + dif[idx + k1],
                                        1,
                                    )
                                ),
                            ],
